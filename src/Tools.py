...
class ConfigManager:
    ...
    def __init__(self, cli_parser):
        ...
        self.arg_config_dict = self.extract_mode_args(self.arg_config_dict, self.args.mode)
        # Get account args from file
        self.acc_config_dict = self.load_toml_config(self.arg_config_dict['acc_config'])
        # Combine all args (into a dict) and update parser
        self.arg_config_all = {**self.arg_config_dict, **self.acc_config_dict}
        self.parser.set_defaults(**self.arg_config_all)
...
class TaskScheduler:
    ...
    def setup_schedule(self):
        ...
        if hour_interval is not None and minute_interval is not None:
            time_interval_min = (hour_interval * 60) + minute_interval
        elif hour_interval is not None:
            time_interval_min = hour_interval * 60
        elif minute_interval is not None:
            time_interval_min = minute_interval
        else:
            time_interval_min = None

        if time_interval_min is not None:
            job = schedule.every(time_interval_min).minutes.do(self.task_function)
            logging.info(f'Scheduled task to run every {time_interval_min} minutes.')
